...
class IndexController extends AbstractActionController
{
	...
	/*
	 * This function is used to generate accessToken when register form is open
	...
	public function registerFormAction() {
		...
		if($this->getRequest()->getMethod() == 'POST') {
			...
			if ((!isset($postedValues['devicetoken'])) || ($postedValues['devicetoken'] == '')) {
				$dataArr[0]['flag'] = "Failure";
				$dataArr[0]['message'] = "Device Token is required.";
				echo json_encode($dataArr);
				exit;
			}
			if ((!isset($postedValues['devicetype'])) || ($postedValues['devicetype'] == '')) {
				$dataArr[0]['flag'] = "Failure";
				$dataArr[0]['message'] = "Device Type is required.";
				echo json_encode($dataArr);
				exit;
			}
			$uniqueToken = $postedValues['deviceToken']."#".uniqid();
			$data['device_token'] = $postedValues['devicetoken'];
			$data['device_type'] = $postedValues['devicetype'];
			$data['access_token'] = $uniqueToken;
			$tokens = new Tokens();
			$tokens->exchangeArray($data);
			$insertedId = $this->getTokensTable()->saveUserToken($tokens);
			$dataArr[0]['flag'] = "Success";
			$dataArr[0]['accesstoken'] = $uniqueToken;
			echo json_encode($dataArr);
			exit;
		} else {
			$dataArr[0]['flag'] = "Failure";
	...
	public function registerAction()
    {
		...
		if($this->getRequest()->getMethod() == 'POST') {
			...
			if ((!isset($postedValues['device_token'])) || ($postedValues['device_token'] == '')) {
				$dataArr[0]['flag'] = "Failure";
				$dataArr[0]['message'] = "Device Token is required.";
				echo json_encode($dataArr);
				exit;
			}
			if ((!isset($postedValues['access_token'])) || ($postedValues['access_token'] == '')) {
				$dataArr[0]['flag'] = "Failure";
			...
			//check for access token against device token
			$token_details = $this->getTokensTable()->getUserToken($postedValues['device_token'], $postedValues['access_token']);
			
			if ($token_details) {
				if (($postedValues['access_token'] != $token_details->access_token)) {
						$dataArr[0]['flag'] = "Failure";
				...
				$dataArr[0]['flag'] = "Failure";
				$dataArr[0]['message'] = "This token has been expired.";
				echo json_encode($dataArr);
				exit;
			}
			$bcrypt = new Bcrypt();
			...
			if($insertedUserId){
				...
				//delete access Token
				$deletedId = $this->getTokensTable()->deleteToken($postedValues['access_token']);
				$dataArr[0]['flag'] = "Success";
	...
	public function getTokensTable(){
		...
		return  $this->tokensTable = $sm->get('User\Model\TokensTable');    
