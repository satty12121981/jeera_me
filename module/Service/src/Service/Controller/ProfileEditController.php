...
class ProfileEditController extends AbstractActionController
{
	...
	public function ProfileEditlistAction(){
	...
	public function groupdetailsAction(){
		...
		if($this->getRequest()->getMethod() == 'POST') {
			...
			$newsfeedsList = $this->getProfileEditTable()->getNewsFeeds($user_id,$type,$group_id,$activity,(int) $limit,(int) $offset);
			if(!empty($newsfeedsList)){
				foreach($newsfeedsList as $list){
					...
					switch($list['type']){
						...
						$SystemTypeData   = $this->getProfileEditTable()->fetchSystemType("Activity");
						...
						case "New Status":
							...
							$SystemTypeData = $this->getProfileEditTable()->fetchSystemType("Discussion");
							$like_details  = $this->getLikeTable()->fetchLikesCountByReference($SystemTypeData->system_type_id,$list['event_id'],$user_id);
							$comment_details  = $this->getCommentTable()->fetchCommentCountByReference($SystemTypeData->system_type_id,$list['event_id'],$user_id); 
							$str_liked_users = '';
							$arr_likedUsers = array();
						...
						case "New Media":
							...
							$SystemTypeData = $this->getProfileEditTable()->fetchSystemType("Media");
							$like_details  = $this->getLikeTable()->fetchLikesCountByReference($SystemTypeData->system_type_id,$list['event_id'],$user_id);
							$comment_details  = $this->getCommentTable()->fetchCommentCountByReference($SystemTypeData->system_type_id,$list['event_id'],$user_id); 
							$str_liked_users = '';
							$arr_likedUsers = array();
    ...
    public function exploreProfileEditAction(){
    	$error = '';
		$request   = $this->getRequest();
		if ($request->isPost()){ 
			$config = $this->getServiceLocator()->get('Config');
			...
			$arr_group_list = '';
			$ProfileEdit = $this->getUserGroupTable()->getmatchProfileEditByuserTags($user_id,$city,$country,$myfriends,$category,(int) $limit,(int) $offset);
			if(!empty($ProfileEdit)){
				foreach($ProfileEdit as $list){
					if (!empty($list['group_photo_photo']))
					$list['group_photo_photo'] = $config['pathInfo']['absolute_img_path'].$config['image_folders']['group'].$list['group_id'].'/medium/'.$list['group_photo_photo'];
					else
					$list['group_photo_photo'] = $config['pathInfo']['absolute_img_path'].'/images/group-img_def.jpg';
				...
				$dataArr[0]['flag'] = "Success";
				$dataArr[0]['ProfileEdit'] = $arr_group_list;
				echo json_encode($dataArr);
				exit;
			}
			else{
				$dataArr[0]['flag'] = "Failure";
				$dataArr[0]['message'] = "No ProfileEdit available.";
				echo json_encode($dataArr);
				exit;
			}
		}else{
	...
	public function getProfileEditTable(){
		$sm = $this->getServiceLocator();
		return  $this->groupTable = (!$this->groupTable)?$sm->get('ProfileEdit\Model\ProfileEditTable'):$this->groupTable;    
	}
	public function getUserGroupTable(){
		$sm = $this->getServiceLocator();
		return  $this->userGroupTable = (!$this->userGroupTable)?$sm->get('ProfileEdit\Model\UserGroupTable'):$this->userGroupTable;    
	}
	...
	public function getGroupMediaTable(){
		$sm = $this->getServiceLocator();
		return  $this->groupMediaTable = (!$this->groupMediaTable)?$sm->get('ProfileEdit\Model\GroupMediaTable'):$this->groupMediaTable;    
    }
