...
class UserProfileTable extends AbstractTableGateway
{
    ...
    } 		public function saveUserProfileApi(UserProfile $user)    {       $data = array(            'user_profile_dob' => $user->user_profile_dob,            'user_profile_about_me'  => $user->user_profile_about_me,			'user_profile_profession'  => $user->user_profile_profession,			'user_profile_profession_at'  => $user->user_profile_profession_at,			'user_profile_user_id'  => $user->user_profile_user_id,			'user_profile_city_id'  => $user->user_profile_city_id,						'user_profile_country_id'  => $user->user_profile_country_id,			'user_address'  => $user->user_address,			'user_profile_current_location'  => $user->user_profile_current_location,			'user_profile_phone'  => $user->user_profile_phone,			 			'user_profile_added_ip_address'  => $user->user_profile_added_ip_address,			'user_profile_modified_timestamp'  => date("Y-m-d H:i:s"),			'user_profile_modified_ip_address'  => $user->user_profile_modified_ip_address		        );		$user_profile_id = (int)$user->user_profile_id;        if ($user_profile_id == 0) {            try {				$this->insert($data);				return $this->adapter->getDriver()->getConnection()->getLastGeneratedValue();			} catch (\Exception $e) {				$dataArr[0]['flag'] = "Failure";				$dataArr[0]['message'] = "Database Error.";				echo json_encode($dataArr);				exit;			}        } else {			            if ($this->getUserProfile($user_profile_id)) {				try {					$this->update($data, array('user_profile_id' => $user_profile_id));				} catch (\Exception $e) {					$dataArr[0]['flag'] = "Failure";					$dataArr[0]['message'] = "Database Error.";					echo json_encode($dataArr);					exit;				}            } else {                throw new \Exception('Form id does not exist');            }        }    } 	
